...
| 2026-02-09 | **Vitest – visi 5 testų failai lūžta:** `TypeError: Cannot read properties of undefined (reading 'on')` ❯ src/test/setup.ts:64:1. Test Files 5 failed, Tests no tests. | Lokalus `npm run test:run`, Windows | P1 | į TODO įrašyta | Reikia gilesnio įsigilinimo. Žr. TODO „Testų infrastruktūra (Vitest/setup)“. Padaryta: setup.ts – process/process.on stub (globalThis, global, vi.stubGlobal); vartotojo aplinkoje vis tiek lūžta. Galimybė: testai rašyti anksčiau nei vėlesnė sistemos dalis – stringai/prielaidos gali būti pasenę; galbūt reikia perrašyti ar atnaujinti testus. |
