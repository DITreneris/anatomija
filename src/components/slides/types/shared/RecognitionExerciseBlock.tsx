...
export function RecognitionExerciseBlock({
  ...
  const [selections, setSelections] = useState<(number | null)[]>(exercise.examples.map(() => null));
  ...
  return (
    <div className={`mt-6 p-6 rounded-xl ${borderAccent} ${bgAccent}`}>
      <h4 className="font-bold text-lg text-gray-900 dark:text-white mb-3">{exercise.title}</h4>
      <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-4">{renderBodyWithBold(exercise.task)}</p>
      <div className="space-y-4 mb-4">
        {exercise.examples.map((ex, exIdx) => {
          ...
          return (
            <div key={exIdx} className="p-4 rounded-lg bg-white dark:bg-gray-800/80 border border-gray-200 dark:border-gray-600">
              <p className="text-gray-800 dark:text-gray-200 font-medium mb-2">
                {exIdx + 1}. {ex}
              </p>
              <div className="flex flex-col gap-2" role="group" aria-label="Pasirink DI visatos sluoksnį">
                {exercise.choices.map((choice, choiceIdx) => {
                  const isSelected = selected === choiceIdx;
                  const showCorrect = selected !== null && choiceIdx === correct;
                  ...
                  return (
                    ...
                    >
                      {disabled && showCorrect && <Check className="w-4 h-4 shrink-0" aria-hidden />}
                      {disabled && showWrong && <X className="w-4 h-4 shrink-0" aria-hidden />}
                      {choice}
                    </button>
                  );
                })}
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 pt-3 border-t border-accent-200 dark:border-accent-800 leading-relaxed">
        <span className="font-bold text-gray-800 dark:text-gray-200">Tikslas: </span>
